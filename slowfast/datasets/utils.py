...
def flatten(frames):
    ...
    assert frames.ndim == 4, "Input tensor should have 4 dimensions."
    x = frames.shape[0]
    c = frames.shape[1]
    size = frames.shape[2]
    ...
    if length != 1:
        ...
        for n in range(1, length // 2 + 1):  
            m = length // n 
            if n*m == length and n==m:  
                break
    total_size = size * n

    for i in range(x//length):
        imgs = frames[i*length:(i+1)*length]
        imgs_flatten = rearrange(imgs, "(n m) c h w -> c (n h) (m w)", n=n, m=m)
        # imgs_flatten = torchvision.transforms.Resize(total_size)(imgs_flatten)
    ...
    imgs_group = imgs_group.permute(1, 0, 2, 3)
